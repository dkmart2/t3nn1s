...
#%%
# how many api rows?
key_cols = ["event_key", "composite_id"]  # keys used for identifying duplicates
...
merged_flag = (
    historical_data
    .merge(
        api_subset[key_cols],   # avoid duplicate column labels
        on=key_cols,
        how="left",
        indicator=True
    )["_merge"]
...
class TennisAbstractScraper:
    ...
    def scrape_stats_overview(self, url):
        ...
        # pull additional metadata from page header
        header_meta = self._extract_match_header(soup)
        match_info.update(header_meta)
...
# %%
# Test Scrapers
url = "https://www.tennisabstract.com/charting/20250701-W-Wimbledon-R128-Mayar_Sherif-Mirra_Andreeva.html"
scraper = TennisAbstractScraper()

print("\n=== SHOT TYPES ===")
for rec in scraper.scrape_shot_types(url):
    print(rec)

print("\n=== SHOT DIRECTION ===")
for rec in scraper.scrape_shot_direction(url):
    print(rec)
...
print("\n=== HEADER ===")
print(scraper._extract_match_header(BeautifulSoup(requests.get(url,headers=scraper.headers).text,"lxml")))

print("\n=== STATS OVERVIEW ===")
for rec in scraper.scrape_stats_overview(url):
    print(rec)

print("\n=== SERVE STATISTICS OVERVIEW ===")
for row in scraper.scrape_serve_statistics_overview(url):
    print(row)

print("\n=== SERVE INFLUENCE ===")
for rec in scraper.scrape_serve_influence(url):
    print(rec)

print("\n=== KEY POINT OUTCOMES ===")
for rec in scraper.scrape_key_point_outcomes(url):
    print(rec)

# POINT OUTCOMES BY RALLY LENGTH
print("\n=== POINT OUTCOMES BY RALLY LENGTH ===")
for rec in scraper.scrape_rally_outcomes(url, debug=False):
    print(rec)

# POINT LOG
print("\n=== POINT LOG ===")
for rec in scraper.scrape_pointlog(url):
    print(rec)

print("\n=== SERVE BREAKDOWN ===")
for rec in scraper.scrape_serve_breakdown(url):
    print(rec)
...
print("\n=== NET POINTS ===")
for row in scraper.scrape_net_points(url):
    print(row)
