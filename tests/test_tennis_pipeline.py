...
def test_full_orchestration(mock_api_integration, mock_scraper, mock_api):
    """FIXED: Test orchestration with comprehensive mocking to avoid API timeouts"""
    print("\n=== TESTING FULL ORCHESTRATION ===")

    # Mock all external dependencies to avoid real API calls
    mock_api.return_value = []  # No API fixtures

    # Mock API integration to return data unchanged (no incremental changes)
    ...
    try:
        ...
        # This should now use mocks instead of real API calls
        ...
        if hist is not None:
            ...
            # Test API integration (should be fast with mocks)
