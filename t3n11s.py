...
def _quick_check_direction(records: list[dict]) -> None:
    """sanity-check: 4 summary + 12 outcome rows per match ⇒ 16 total"""
    grp = {}
    for r in records:
        key = (r["Player_canonical"], r["category_group"])
        grp.setdefault(key, []).append(r)

    for (player, g), rows in grp.items():
        expected = 4 if g == "direction_summary" else 12
        if len(rows) != expected:
            raise AssertionError(
                f"{player} {g}: {len(rows)} rows (expected {expected})"
            )
