#!/usr/bin/env python3
"""
Scrape ONLY the recent 2025 missing Tennis Abstract matches (not historical ones)
"""
...
def get_recent_missing_urls():
    """Get only the 2025 missing URLs by comparing discovered vs cached"""
    print("🔍 IDENTIFYING RECENT MISSING URLs")
    print("="*50)
    ...
    try:
        ...
        print(f"✅ Discovered {len(discovered_urls)} URLs from June 10+ 2025")
    ...
    print(f"🆕 Found {len(missing_urls)} missing URLs from 2025")
    ...
    for i, (match_date, url) in enumerate(dated_urls[:10]):
        match_name = url.split('/')[-1].replace('.html', '')[:60]
        print(f"  {i+1:2d}. {match_date}: {match_name}")
    
    return [url for _, url in dated_urls]

def scrape_recent_missing_matches():
    """Scrape only the recent 2025 missing Tennis Abstract matches"""
    print("🎾 SCRAPING RECENT MISSING TENNIS ABSTRACT MATCHES")
    print("="*70)
    print("Goal: Scrape missing 2025 matches for complete coverage")
    print()
    
    # Get missing URLs (filtered for 2025)
    missing_urls = get_recent_missing_urls()
    
    if not missing_urls:
        print("✅ No missing URLs found - cache appears complete")
        return pd.DataFrame(), 0
    
    print(f"\n🚀 STARTING SCRAPE OF {len(missing_urls)} RECENT MATCHES")
    print("-" * 50)
    ...
    cache_dir = Path('/Users/danielkim/Desktop/t3nn1s/tennis_abstract_recent')
    ...
    if successful_urls:
        ...
        if all_matches:
            ...
            matches_file = cache_dir / f'recent_missing_matches_{timestamp}.parquet'
            matches_df.to_parquet(matches_file, index=False)
            print(f"💾 Matches: {matches_file}")
        
        if all_records:
            records_df = pd.DataFrame(all_records)
            records_file = cache_dir / f'recent_missing_records_{timestamp}.parquet'
        ...
        print(f"\n🎯 TENNIS ABSTRACT COVERAGE UPDATE!")
...
if __name__ == "__main__":
    matches_df, total_records = scrape_recent_missing_matches()
    
    if not matches_df.empty or total_records > 0:
        print(f"\n🚀 SUCCESS!")
        print(f"Recent Tennis Abstract matches scraped successfully!")
        print(f"Ready for integration with complete dataset")
    else:
        print(f"\n⚠️  No recent matches found to scrape")
        print(f"May already have complete 2025 coverage")